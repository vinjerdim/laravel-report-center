...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('citizen_id')}">
    <label for="citizen_id" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.report.columns.citizen_id') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
            <select v-model="form.citizen_id" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('citizen_id'), 'form-control-success': fields.citizen_id && fields.citizen_id.valid}" id="citizen_id" name="citizen_id">
                <option v-for="citizen in form.citizens" :value="citizen.id">
                    @{{ citizen.name }}
                </option>
            </select>
