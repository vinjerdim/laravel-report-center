...
@section('title', 'Login as Officer')
	        ...
	        <div class="col-md-6 col-lg-5">
				<div class="card">
					<div class="card-block">
						<auth-form
								:action="'{{ url('/officer/login') }}'"
								:data="{}"
								inline-template>
							<form class="form-horizontal" role="form" method="POST" action="{{ url('/officer/login') }}" novalidate>
								{{ csrf_field() }}
								<div class="auth-header">
									<h1 class="auth-title"> Login </h1>
									<p class="auth-subtitle"> as Officer </p>
								</div>
								<div class="auth-body">
									@include('officer.auth.includes.messages')
									<div class="form-group" :class="{'has-danger': errors.has('email'), 'has-success': fields.email && fields.email.valid }">
										...
										<div class="input-group input-group--custom">
											...
											<input type="text" v-model="form.username" v-validate="'required'" class="form-control" :class="{'form-control-danger': errors.has('username'), 'form-control-success': fields.username && fields.username.valid}" id="username" name="username" placeholder="Username">
										</div>
