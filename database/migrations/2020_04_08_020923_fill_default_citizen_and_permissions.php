...
class FillDefaultCitizenAndPermissions extends Migration
{
    /**
     ...
     * @var string
     */
    protected $password = 'apapun';

    /**
     * FillDefaultCitizenUserAndPermissions constructor.
     */
    public function __construct()
    {
        $this->guardName = config('citizen-auth.defaults.guard');
    ...
    public function up()
    {
        if ($this->userClassName === null) {
            throw new RuntimeException('Citizen user model not defined');
        }
        DB::transaction(function () {
            ...
            foreach ($this->users as $user) {
                ...
                if ($userItem === null) {
                    ...
                    Citizen::find($userId)->addMedia(storage_path() . '/images/avatar.png')
                        ->preservingOriginal()
    ...
    public function down()
    {
        ...
        DB::transaction(function () {
            foreach ($this->users as $user) {
                $userItem = DB::table($this->userTable)->where('username', $user['username'])->first();
                if ($userItem !== null) {
                    Citizen::find($userItem->id)->media()->delete();
